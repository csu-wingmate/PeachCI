...
for i in $(seq 1${worker_num}) ; do
    ...
    # Run the worker in the new network namespace
    ip netns exec ${netns} bash -c "
        ...
        timeout ${TIMEOUT} mono ${FUZZER_PATH}/protocol-fuzzer-ce/output/linux_x86_64_release/bin/peach.exe \
            ${container_pit_path} &
    "
done
